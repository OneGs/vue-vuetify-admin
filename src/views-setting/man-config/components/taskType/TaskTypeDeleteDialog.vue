...
import { taskTypeDelete } from "@/requests/apis/zRisker/taskType";
import EditEmit from "@cps/rule-dailog/mixins/editEmit";
import RuleDialog from "@cps/rule-dailog/index.vue";

@Component({
  name: "TaskTypeDeleteDialog",
})
export default class TaskTypeDeleteDialog
  ...
  implements IDeleteDialog
{
  ...
  @Prop({ type: Object, default: () => ({}) }) item!: taskTypeResponse;

  async onSubmit(): Promise<void> {
    this.loading = true;

    if (!(await taskTypeDelete({ ids: [this.item.id] }))) {
      this.loading = false;
