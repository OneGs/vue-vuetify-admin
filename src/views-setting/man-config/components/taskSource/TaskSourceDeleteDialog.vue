...
import { taskSourceResponse } from "@/types/taskSource";
import { taskSourceDelete } from "@/requests/apis/zRisker/taskSource";
import EditEmit from "@cps/rule-dailog/mixins/editEmit";
import RuleDialog from "@cps/rule-dailog/index.vue";

@Component({
  name: "TaskSourceDeleteDialog",
})
export default class TaskSourceDeleteDialog
  ...
  implements IDeleteDialog
{
  ...
  @Prop({ type: Object, default: () => ({}) }) item!: taskSourceResponse;

  async onSubmit(): Promise<void> {
    this.loading = true;

    if (!(await taskSourceDelete({ ids: [this.item.id] }))) {
      this.loading = false;
